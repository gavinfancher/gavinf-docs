...
The pieces come out of `.env` at the repo root:

| Placeholder | `.env` key | Example |
|---|---|---|
| `<host>` | `PROXMOX_HOST` | `100.x.y.z` (Tailscale IP — you must be on the tailnet) |
...
A node might return `500` (`homecloud`, running), `8001`
(`tpl-homecloud-base`, template), and `9100` (`cloudimg-debian-12`, template).
...
| Nonexistent vmid | `500` | `{"message":"Configuration file 'nodes/pve/qemu-server/999.conf' does not exist\n","data":null}` |
